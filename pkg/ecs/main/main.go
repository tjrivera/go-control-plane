...
// Package main contains the ecs driver for testing xDS manually.
package main
...
import (
	...
	"github.com/envoyproxy/go-control-plane/pkg/ecs"
	"github.com/envoyproxy/go-control-plane/pkg/ecs/resource"
...
func main() {
	...
	// start upstream
	go ecs.RunHTTP(ctx, upstreamPort)

	// create a cache
	signal := make(chan struct{})
	cb := &callbacks{signal: signal}
	config := cache.NewSnapshotCache(mode == resource.Ads, ecs.Hasher{}, logger{})
	srv := server.NewServer(config, cb)
	als := &ecs.AccessLogService{}
	...
	go ecs.RunAccessLogServer(ctx, als, alsPort)
	go ecs.RunManagementServer(ctx, srv, port)
	go ecs.RunManagementGateway(ctx, srv, gatewayPort)
